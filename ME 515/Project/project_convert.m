...
% was successful, a parking orbit altitude of *190km* will be used. It requires 
% less total velocity impulse to obtain the orbit via Hohmann Transfer compared 
% to higher LEOs, while still being high enough to avoid atmospheric drag.
...
rem = 384400;                                   % Apogee Radius of Transfer Ellipse, which is approximately the 
...
T1 = 2*pi*sqrt((a1^3)/mu)           % Period of Translunar Trajectory in Seconds
...
e2mag = norm(e2)    % eccentricity of hyperbola
%% 
...
T.system_check_end = T.parking + hours(T.sys);
T.TranslunarInjection = T.system_check_end + hours(T.sys_to_tli);
T.Perilune = T.TranslunarInjection + hours(T.tli) + hours(T.perilune);
T.EarthArrival = T.TranslunarInjection + hours(T.translunar);
...
disp("Altitude of Perilune Approach = " + m.z + " km")
%%
